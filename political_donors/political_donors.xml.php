...
$largestDonation = 1469619; //  select "AmountPaid" from political_donations order by "AmountPaid" desc limit 1
$largestDonor = 9379000; // select sum("AmountPaid") as total from political_donations group by "DonorClientNm" order by total desc limit 1
$largestRecipient = 20061764; // select sum("AmountPaid") as total from political_donations group by "RecipientClientNm" order by total desc limit 1
...
foreach ($result->fetchAll() as $row) {
    if ((strpos($row['RecipientClientNm'], "Labor") !== false || strpos($row['RecipientClientNm'], "ALP") !== false) && strpos($row['RecipientClientNm'], "Democratic") === false)
        $party = "labor";
    else if (strpos($row['RecipientClientNm'], "Liberal") !== false)
        $party = "liberal";
    else if (strpos($row['RecipientClientNm'], "National") !== false)
        $party = "national";
    else
        $party = "other";
    addDonationRecipientNode($row['RecipientClientNm'], $party, $row['AmountPaid']);
}
$result = $dbConn->prepare('select "DonorClientNm",sum("AmountPaid") as "AmountPaid"
                        from political_donations group by "DonorClientNm"');
$result->execute();
foreach ($result->fetchAll() as $row) {
    addDonorNode($row['DonorClientNm'], $row['AmountPaid']);
}
$result = $dbConn->prepare('select "DonorClientNm","RecipientClientNm",sum("AmountPaid") as "AmountPaid"
