...
$suppliers = $dbConn->prepare("
 SELECT supplierName, supplierABN, value, count(1) as count
FROM `contractnotice`
WHERE agencyName = ?
AND childCN = 0
...
foreach($suppliers->fetchAll() as $row) {
  ...
  if (!$exists) {
    ...
    $node->addAttribute("tooltip", "$" . number_format($row['value'], 2, '.', ',') . " (" . number_format((($row['value'] / $agencyTotalValue) * 100) , 2) . "% of all contract expenditure) \n" . $row['count'] . " contracts (" . number_format((($row['count'] / $agencyTotalContracts) * 100) , 2) . "% of all 
contracts)");
	...
	$valueNode = $txnNode->addChild('value', $row['value']);
